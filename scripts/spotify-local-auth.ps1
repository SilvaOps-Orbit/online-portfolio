param(
  ...
  [string]$Scopes = $(if ($env:SPOTIFY_SCOPES) { $env:SPOTIFY_SCOPES } else { "user-read-private user-read-currently-playing user-read-playback-state user-read-recently-played user-top-read user-follow-read playlist-read-private" })
)
