...
function capsuleBriefing(value?: string, date?: string, detail?: string): { title: string; body: string } {
  const title = capsuleTitle(value);
  const when = date ? ` around ${new Date(`${date}T00:00:00`).toLocaleDateString("en-AU", { day: "numeric", month: "long", year: "numeric" })}` : "";
  if (cleanInsightCopy(detail)) return { title, body: cleanInsightCopy(detail) };
  const briefings: Record<string, string> = {
    "Listening milestone": `Spotify marked a notable listening checkpoint${when}. It reflects a meaningful pattern in the private archive, rather than a public achievement or a single song.`,
    "Unexpected combination": `Spotify identified an unusual mix of artists, tracks, or listening habits${when}. The export does not include the exact pairing, so this keeps the signal useful without exposing private listening data.`,
    "Listening streak": `Spotify detected a sustained run of listening activity${when}. It is a rhythm marker from the private archive, not a count of consecutive days shown publicly.`,
    "Early discovery": `Spotify recorded a discovery moment${when}, showing a newer artist or sound entering the listening pattern.`
  };
  return { title, body: briefings[title] || `Spotify saved this as a music moment${when}. The public archive retains the category and date only.` };
}
...
function MomentsView({ archive }: { archive?: SpotifyArchiveData | null }) {
  ...
  const selectedBriefing = selectedMoment ? capsuleBriefing(selectedMoment.title, selectedMoment.date, selectedMoment.note) : null;
  const max = Math.max(1, Number(heatmap?.maxMinutes || 0));
  return <div className="spotify-moments"><div className="spotify-wrapped-year"><div><span>Spotify Wrapped</span><strong>{wrapped?.year || "Archive"}</strong><small>{Number(wrapped?.minutes || 0).toLocaleString("en-AU")} minutes listened</small></div><div className="spotify-wrapped-metrics"><span><b>{Number(wrapped?.uniqueArtists || 0).toLocaleString("en-AU")}</b> artists</span><span><b>{Number(wrapped?.uniqueTracks || 0).toLocaleString("en-AU")}</b> tracks</span><span><b>{Number(wrapped?.uniqueGenres || 0).toLocaleString("en-AU")}</b> genres</span></div></div><div className="spotify-heatmap-wrap"><div className="insight-subhead"><span>Listening hour / day</span><small>archive playtime intensity</small></div>{heatmap?.cells?.length ? <div className="spotify-heatmap" style={{ gridTemplateColumns: `54px repeat(${heatmap.hours?.length || 24}, minmax(10px, 1fr))` }}><span className="spotify-heatmap-corner" />{heatmap?.hours?.map((hour) => <span key={hour} className="spotify-heatmap-hour">{hour % 3 === 0 ? String(hour).padStart(2, "0") : ""}</span>)}{heatmap.cells.map((day, dayIndex) => <>{<span className="spotify-heatmap-day" key={`day-${dayIndex}`}>{heatmap.labels?.[dayIndex]}</span>}{day.map((minutes, hour) => <span key={`${dayIndex}-${hour}`} className="spotify-heatmap-cell" style={{ "--heat": Math.max(0.08, minutes / max) } as React.CSSProperties} title={`${heatmap.labels?.[dayIndex]} ${String(hour).padStart(2, "0")}:00 - ${minutes} minutes`} />)}</>)}</div> : <p className="insight-empty">Listening rhythm will appear after the archive snapshot is refreshed.</p>}</div><div className="spotify-capsule"><div className="insight-subhead"><span>Music moments</span><small>{moments.length} saved moments · select one for intel</small></div><div className="spotify-capsule-list">{moments.map((moment, index) => { const briefing = capsuleBriefing(moment.title, moment.date, moment.note); const isSelected = moment.id === selectedMomentId; return <button type="button" className={isSelected ? "is-selected" : ""} key={moment.id || `${moment.date}-${index}`} aria-pressed={isSelected} onClick={() => setSelectedMomentId(isSelected ? "" : moment.id || "")}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{briefing.title}</strong><small>{moment.entities?.join(" · ") || moment.date || "Spotify Sound Capsule moment"}</small></div><time>Intel</time></button>; })}</div>{selectedBriefing && <article className="spotify-capsule-briefing" aria-live="polite"><span>Moment briefing</span><strong>{selectedBriefing.title}</strong><p>{selectedBriefing.body}</p>{selectedMoment?.date && <small>Captured {new Date(`${selectedMoment.date}T00:00:00`).toLocaleDateString("en-AU", { day: "numeric", month: "long", year: "numeric" })}</small>}<button type="button" onClick={() => setSelectedMomentId("")}>Close briefing</button></article>}</div></div>;
}
